---
layout: cover
event: Web Summer Camp 2026
speakers:
  - name: Indermohan Singh
    role: Lead Developer Advocate · Dynatrace
    avatar: /team/imsingh.png
  - name: Dani Coll
    role: Lead Developer Advocate · Dynatrace
    avatar: /team/dani.jpeg
---

# Seeing the Unseen
## *Observability* for your JavaScript apps

<!--
Welcome everyone to Observability in Action. Introduce yourselves and the event.
-->

---
layout: default
heading: Agenda
---

<Grid :cols="2" gap="lg" rowGap="lg" class="mt-6">
  <Card v-click variant="filled">
    <div class="text-lg font-semibold mb-1">01 · Introduction to Modern Observability</div>
    <div class="text-sm opacity-70">What observability means and why it matters today</div>
  </Card>
  <Card v-click variant="filled">
    <div class="text-lg font-semibold mb-1">02 · Three Pillars Deep Dive</div>
    <div class="text-sm opacity-70">Logs, Metrics, and Traces in practice</div>
  </Card>
  <Card v-click variant="filled">
    <div class="text-lg font-semibold mb-1">03 · Distributed Tracing</div>
    <div class="text-sm opacity-70">Following requests across services end-to-end</div>
  </Card>
  <Card v-click variant="filled">
    <div class="text-lg font-semibold mb-1">04 · Best Practices and Wrap</div>
    <div class="text-sm opacity-70">Hands-on patterns, Q&A, and takeaways</div>
  </Card>
</Grid>

<p v-click class="mt-6 text-sm opacity-60 flex items-center gap-2"><span class="i-ph-coffee-bold w-5 h-5 shrink-0 text-amber-300" /> Two 1h15m sessions with a 30min coffee break in between — pick the one that works for you.</p>

<!--
Walk through the agenda. Two identical sessions separated by a coffee break — attendees attend one.
-->
